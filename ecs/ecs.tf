resource "aws_ecs_cluster" "test-cluster" {
  name = "myapp-cluster-${var.customer_name}"
}

data "template_file" "testapp" {
  template = file("${path.module}/templates/image/image.json")
...
resource "aws_ecs_task_definition" "test-def" {
  family                   = "testapp-task-${var.customer_name}"
...
resource "aws_ecs_service" "test-service" {
  name            = "testapp-service-${var.customer_name}"
